...
import { getFacultyAttendance } from "../../api/services/director";
...
const FacultyFields = forwardRef(({ isHoD = false, department = '' }: FacultyFieldsProps, ref) => {
  const [selectedProfessor, setSelectedProfessor] = useState('');
  const [selectedSubject, setSelectedSubject] = useState('');
  const [employeeCodeInput, setEmployeeCodeInput] = useState('');
  const [selectionMethod, setSelectionMethod] = useState(isHoD ? 'department' : 'employee');
  ...
  }, [isHoD, department, setProfessors]);
  ...
  const handleSubmit = async () => {
    ...
    if (selectionMethod === 'employee') {
      ...
      if (isHoD) {
        // Verify employee is in HOD's department
        const facultyInDept = professors.find((p: Professor) => p.employeeCode === employeeCode);
        if (!facultyInDept) {
          toast.showError("Faculty member not in your department", "faculty-not-in-dept");
          return null;
        }
      }
    ...
    <div className="space-y-8 max-w-5xl mx-auto p-6 bg-gray-50 rounded-xl shadow-sm">
      {isHoD && (
        <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
          <p className="text-sm text-blue-700">
            Viewing faculty from <span className="font-semibold">{department}</span> department only
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <SelectionMethodCard
          ...
          description={isHoD ? "View faculty members from your department" : "Select faculty members from specific departments"}
          selected={selectionMethod === 'department'}
          onClick={() => !isHoD && setSelectionMethod('department')}
          disabled={isHoD}
        />
        <SelectionMethodCard
          method="employee"
          ...
          description={isHoD ? "Search faculty by code within your department" : "Directly search using faculty employee code"}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {!isHoD && (
              <SelectField
                ...
                onChange={() => {}}
                icon={Building2}
                tooltip="Your department selection"
                options={[
                  <option key="dept" value={department}>{department}</option>
                ]}
                disabled={true}
              />
            )}
                ...
                ...(loadingProfessors
                  ...
                  : professors.map((prof: Professor, index: number) => (
                    <option key={`${prof.employeeCode}-${index}`} value={prof.employeeCode}>
                      {`${prof.name} (${prof.abbreviation || prof.employeeCode})`}
                    </option>
                  ))
                )
              ]}
          ...
          <div className="max-w-md">
            <div className="space-y-2">
              <label htmlFor="employeeCodeInput" className="flex items-center space-x-2 text-sm font-medium text-gray-700">
                ...
                {isHoD && (
                  <Tooltip content={`Must be from ${department} department`}>
                    <HelpCircle className="w-4 h-4 text-gray-400 hover:text-gray-600 cursor-help transition-colors" />
                  </Tooltip>
                )}
              </label>
